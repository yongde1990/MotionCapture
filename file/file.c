...
static int isDir(char *strFilePath, char *strFileName)
{
    ...
    strTmp[255] = '\0';
/* 通过文件名strTmp获取文件信息，并保存在tStat所指的结构体stat中*/
/*S_ISDIR 只是一个 macro,判断文件是否是目录*/
...
int GetDirContents(char *strDirName, PT_DirContent **pptDirContents, int *piNumber)	
{
	...
	*int scandir(const char *dir,struct dirent **namelist,int (*filter)(const void *b),
       ...
       *函数scandir扫描dir目录下以及dir子目录下满足filter过滤模式
       *的文件，返回的结果是compare函数经过排序的，并保存
       *在 aptNameList中。注意namelist是通过malloc动态分配内存的,所以
       *在使用时要注意释放内存。
	*/
